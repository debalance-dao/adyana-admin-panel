<x-layout.default>
    <div x-data="striped" class="panel">
        <div class="flex justify-between ">
            <div class="mb-5 text-lg font-bold dark:text-white-light">Project</div>
            <div class="mb-5">
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("striped", () => ({
                projects: @json($project->toArray()),
                dataTable: null,
                init() {
                    const editprojectRoute = "{{ route('projects.edit', ['project' => ':id']) }}";
                    const tableOptions = {
                        data: {
                            headings: ["No", "Name", "Funds Target *Ady", "Description",
                                "Days Target", "Action"
                            ],
                            data: this.projects.map((project, index) => {
                                const editUrl = editprojectRoute.replace(':id', project.id);
                                return [
                                    index + 1,
                                    project.name,
                                    project.funds_target,
                                    ...
                                    <button @click.prevent="confirmAndDelete(${project.id})" type="button" class="hover:text-danger"><svg x-tooltip="Delete" data-theme="danger" data-delay='500' width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                async confirmAndDelete(projectId) {
                    ...
                    if (isConfirmed) {
                        this.deleteIngredient(projectId);
                    }
                },

                async deleteIngredient(projectId) {
                    const csrfToken = document.querySelector('meta[name="csrf-token"]')
                        .getAttribute('content');
                    const rute = "{{ route('projects.destroy', ':id') }}";
                    const url = rute.replace(':id', projectId);
                    try {
                ...
                removeprojectFromList(projectId) {
                    const index = this.projects.findIndex(data => data.id === projectId);
                    if (index !== -1) {
