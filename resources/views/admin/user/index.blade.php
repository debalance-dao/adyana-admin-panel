<x-layout.default>
    <div x-data="striped" class="panel">
        <div class="flex justify-between ">
            <div class="mb-5 text-lg font-bold dark:text-white-light">Users</div>
            <div class="mb-5">
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("striped", () => ({
                ...
                dataTable: null,
                init() {
                    const editUserRoute = "{{ route('users.edit', ['user' => ':id']) }}";
                    const tableOptions = {
                        data: {
                            headings: ["No", "Name", "Email", "Aksi"],
                            data: this.users.map((user, index) => {
                                const editUrl = editUserRoute.replace(':id', user.id);
                                return [
                                    index + 1,
                                    user.name,
                                    user.email,
                                    ...
                                    <button @click.prevent="confirmAndDelete(${user.id})" type="button" class="hover:text-danger"><svg x-tooltip="Delete" data-theme="danger" data-delay='500' width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                async confirmAndDelete(userId) {
                    ...
                    if (isConfirmed) {
                        this.deleteIngredient(userId);
                    }
                },

                async deleteIngredient(userId) {
                    const csrfToken = document.querySelector('meta[name="csrf-token"]')
                        .getAttribute('content');
                    const rute = "{{ route('users.destroy', ':id') }}";
