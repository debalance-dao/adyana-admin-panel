<x-layout.default>
            ...
            <div class="panel">
                ...
                <div>
                    ...
                    <form class="space-y-5 dark:text-white-dark" @submit.prevent="submitForm4()">
                        ...
                        <div :class="[isSubmitForm4 ? (validated.project_id ? 'has-error' : 'has-success') : '']">
                            <label for="selectproject_id">Project</label>
                            <select id="selectproject_id" placeholder="Choose Project" class="text-white-dark"
                                x-model="form4.project_id">
                                @foreach ($project as $prcjt)
                                    <option value="{{ $prcjt->id }}">{{ $prcjt->name }}
                                    </option>
                                @endforeach
                            </select>
                            <template x-if="isSubmitForm4 && !validated.project_id">
                                <p class="text-[#1abc9c] mt-1">Looks Good!</p>
                            </template>
                            <template x-if="isSubmitForm4 && validated.project_id">
                                <p class="text-danger mt-1">
                                    <span x-text="validated.project_id"></span>
                                </p>
                            </template>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div
                                ...
                                <input id="customlname" type="text" placeholder="Enter name allocation"
                                    class="form-input" x-model="funds.name">
                            ...
                            <div
                                ...
                                <input id="customlpercentage" type="number" placeholder="Enter percentage"
                                    class="form-input" x-model="funds.percentage">
                        ...
                        <button type="submit" class="btn btn-primary !mt-12">Submit Form</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function(e) {
            ...
            NiceSelect.bind(document.getElementById("selectproject_id"), options);
        });
    </script>

    <script>
        let funds_collection = JSON.parse(localStorage.getItem('funds_collection')) || [];
        ...
        document.addEventListener("alpine:init", () => {
            Alpine.data("form", () => ({
                ...
                saveFundsAllocation() {
                    if (!this.funds.name || !this.funds.percentage) {
                        // this.isSubmitForm4 = true;
                        this.validated.lname = this.funds.name ? '' : 'name is required';
                        this.validated.lpercentage = this.funds.percentage ? '' :
                ...
                sendFormData() {
                    ...
                    this.funds_collection.forEach((funds, index) => {
                        formData.append(`funds[${index}][name]`, funds.name);
                        formData.append(`funds[${index}][percentage]`, funds.percentage);
                    });
